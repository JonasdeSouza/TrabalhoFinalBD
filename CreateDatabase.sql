...
CREATE TABLE IF NOT EXISTS `mydb`.`TipoEndereco` (
  ...
  `descricao` VARCHAR(45) COLLATE 'Default Collation' NOT NULL COMMENT 'Descrição do tipo de endereço\n\nResidencial e comercial',
...
CREATE TABLE IF NOT EXISTS `mydb`.`TipoFluxo` (
  `idTipoFluxo` INT NOT NULL AUTO_INCREMENT COMMENT 'Identificador único de tipo de fluxo',
...
CREATE TABLE IF NOT EXISTS `mydb`.`Semestre` (
  ...
  INDEX `fk_Semestre_Mencao1_idx` (`Mencao_idMencao` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Semestre_Mencao1`
    FOREIGN KEY (`Mencao_idMencao`)
    REFERENCES `mydb`.`Mencao` (`idMencao`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `mydb`.`MateriasObrigatoriasCurso` (
  `Disciplina_idDisciplina` INT NOT NULL AUTO_INCREMENT,
  `Curso_idCurso` INT NOT NULL,
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (1, 'Cálculo 1', 6, 'Funções de uma variável real, limite e continuidade, derivada, integral, aplicações da integral.');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (2, 'Cálculo 2', 6, 'Sequências e séries numéricas séries de potências fórmula de Taylor equações diferenciais ordinárias de 1ª ordem equações diferenciais ordinárias lineares o método da série de potências a transformada de Laplace sistemas lineares de equações diferenciais ordinárias de 1ª ordem.');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (3, 'Introdução à Engenharia', 2, 'A estrutura da Universidade de Brasília. A estrutura do Curso de Engenharia. Técnicas de administração de tempo. Técnicas de estudo. Noções de Engenharia Automotiva. Noções de Engenharia Eletrônica. Noções de Engenharia de Energia. Noções de Engenharia de Software. Noções de Engenharia Aeroespacial');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (4, 'Algoritmos e Programação de Computadores', 6, 'Princípios fundamentais de construção de programas. Construção de algoritmos e sua representação em pseudocódigo e linguagens de alto nível. Noções de abstração. Especificação de variáveis e funções. Testes e depuração. ');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (5, 'Fisica 1', 4, 'Módulos 1: Unidades e grandezas físicas 2: Vetores 3: Movimento retilineo 4: Movimento em duas e três dimensoes 5: Leis de Newton do movimento 6: Aplicação das Leis de Newton 7');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (6, 'Probabilidade e Estatística', 4, 'Conceitos e noções fundamentais. Variáveis aleatórias. Distribuições das Variáveis aleatórias. Intervalo de confiança. Teste de hipóteses. Erros do Tipo I/II. ');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (7, 'Estruturas de Dados', 4, ' Recursividade- Ponteiros e alocação dinâmica de memória- Estruturas lineares. Arrays. Listas. Filas. Pilhas- Introdução à Complexidade computacional e notação Big-O');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (8, 'Mecânica de Vôo', 4, 'Equacionamento do movimento de uma aeronave considerada como corpo rígido: análise dinâmica e cinemática Conceitos fundamentais da dinâmica e controle de atitude de aeronaves');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (9, 'Eletrônica Veicular', 4, '1. Introdução sistemas elétricos automotivos 2. Componentes automotivos básicos 3. Baterias automotivas 4. Sistema de carregamento 5. Sistemas de partida do motor');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (10, 'Gerência de Redes e Sistemas', 2, 'Princípios, Organização, Processos, Atividades, Áreas e Métodos de Administração de Rede. Tecnologias de Suporte à Gerência de Rede');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (11, 'Fundamentos de Sistemas', 4, 'Definições e aplicações.Metodologias de desenvolvimento de sistemas embarcados.Interfaceamento analógico e digital ');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (12, 'Sistemas Hidroelétricos', 5, 'Energia Hidráulica: Conceitos BásicosCentrais HidroelétricasEstudos HidrológicosSistemas de Baixa e Alta PressãoTurbinas HidráulicasEstudos Sociais, Econômicos e Ambientais');
INSERT INTO `mydb`.`Disciplina` (`idDisciplina`, `nome`, `quantidadeCreditos`, `ementa`) VALUES (13, 'Processamento de Sinais', 4, 'Sinais e sistemas no domínio do tempoSinais e sistemas no domínio da frequência');

COMMIT;


-- -----------------------------------------------------
-- Data for table `mydb`.`TipoSemestre`
-- -----------------------------------------------------
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`TipoSemestre` (`idTipoSemestre`, `descricao`) VALUES (1, 'Normal');
INSERT INTO `mydb`.`TipoSemestre` (`idTipoSemestre`, `descricao`) VALUES (2, 'Verão');
...
START TRANSACTION;
USE `mydb`;
INSERT INTO `mydb`.`Status` (`idStatus`, `descricao`) VALUES (1, 'Planejado');
INSERT INTO `mydb`.`Status` (`idStatus`, `descricao`) VALUES (2, 'Cursado');
INSERT INTO `mydb`.`Status` (`idStatus`, `descricao`) VALUES (3, 'Cursando');
